<template>
    <div>
      ...
      <nav-bread><span>地址</span></nav-bread>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFiltle" :class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                    <dl>
                      <dt>{{ item.userName }}</dt>
      ...
      <modal :mdShow="mdDelAddress" @close="closeModal">
        <p slot="message">是否要删除此地址？</p>
        <div slot='btnGroup'>
          <a href="javascript:;" class="btn btn-m" @click="delAddressComfirm">确认</a>
          <a href="javascript:;" class="btn btn-m" @click="closeModal">取消</a>
        </div>
      </modal>
      <modal :mdShow="mdAddNewAddr" @close="closeModal">
        <p slot="title">新地址</p>
        <div slot="message">
          <ul>
            <li class="regi_form_input">
              <input type="text" tabindex="1" name="userName" class="regi_login_input"  placeholder="收件人姓名"  ref="userName">
            </li>
            <li class="regi_form_input">
              <input type="text" tabindex="1" name="streetName"  class="regi_login_input" placeholder="地址" ref="streetName">
            </li>
            <li class="regi_form_input">
              <input type="text" tabindex="1" name="postCode"  class="regi_login_input" placeholder="邮政编码" ref="postCode">
            </li>
            <li class="regi_form_input">
              <input type="text" tabindex="1" name="tel"  class="regi_login_input" placeholder="收件人电话号码" ref="tel">
            </li>
          </ul>
        </div>
        <div slot='btnGroup'>
          <a href="javascript:;" class="btn btn-m" @click="closeModal">取消</a>
          <a href="javascript:;" class="btn btn-m" @click="addAddressConfirm">保存</a>
        </div>
      </modal>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      setDefault(addressId){
        axios.post("/users/setDefault",{
          ...
          if(res.data.status=="0"){
            this.init()
            console.log("setDefault suc")
          }
        })
      },
      deleteAddress(addressId){
        this.mdDelAddress = true
      ...
      closeModal(){
        this.mdDelAddress = false
        this.mdAddNewAddr = false
      },
      addAddress(){
        this.mdAddNewAddr = true
      },
      addAddressConfirm(){
        axios.post("users/address/add",{
          ...
          if(res.data.status=="0"){
            this.mdAddNewAddr = false
            this.init()
            console.log("add new address:suc")
          }
          else{
